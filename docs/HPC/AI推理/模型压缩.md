...
![[Pasted image 20230604233320.png]]
神经网络模型特点：
- 数据参数量大 ；
- 计算量大；
- 内存占用大； 
...
- 功耗降低；
- ~~提升芯片面积~~；
![[Pasted image 20230604233456.png]]
...
量化方法
![[Pasted image 20230604233757.png]]
...
![[Pasted image 20230604233954.png]]

线性量化可分为对称量化和非对称量化
![[Pasted image 20230604234020.png]]

量化原理

要弄懂模型量化的原理，就要弄懂它的数据映射关系，浮点与定点数据的转换公式如下：
	- ![[Pasted image 20230604234101.png]]       ![[Pasted image 20230604234113.png]]
...
![[Pasted image 20230604234149.png]]        ![[Pasted image 20230604234159.png]]
...
量化算法原始浮点精度数据与量化后 INT8 数据的转换如下：![[Pasted image 20230604234240.png]]

确定后通过原始float32高精度数据计算得到uint8数据的转换即为如下公式所示：![[Pasted image 20230604234248.png]]

若待量化数据的取值范围为 [Xmin, Xmax]，则 scale 的计算公式如下：![[Pasted image 20230604234252.png]]


offset的计算方式如下：![[Pasted image 20230604234259.png]]
...
![[Pasted image 20230604234501.png]]
...
伪量化节点：正向传播 Forward
- 为了求得网络模型tensor数据精确的Min和Max值，因此在模型训练的时候插入伪量化节点来模拟引入的误差，得到数据的分布。对于每一个算子，量化参数通过下面的方式得到：![[Pasted image 20230604234612.png]]

- 正向传播的时候fake quant节点对数据进行了模拟量化规约的过程，如下图所示：![[Pasted image 20230604234636.png]]

伪量化节点：反向传播 Backward
- 按照正向传播的公式，如果方向传播的时候对其求导数会导致权重为0，因此反向传播的时候相当于一个直接估算器：![[Pasted image 20230604234714.png]]

- 最终反向传播的时候fake quant节点对数据进行了截断式处理，如下图所示：![[Pasted image 20230604234730.png]]
...
伪量化节点：插入方式
![[Pasted image 20230604234828.png]] ![[Pasted image 20230604234836.png]]
...
PTQ Dynamic 算法流程 ![[Pasted image 20230604235332.png]]

#### PTQ Static
- 静态离线量化（Post Training Quantization Static, PTQ Static）
- 同时也称为校正量化或者数据集量化。使用少量无标签校准数据，核心是计算量化比例因子。使用静态量化后的模型进行预测，在此过程中量化模型的缩放因子会根据输入数据的分布进行调整。![[Pasted image 20230604235408.png]]
- 静态离线量化的目标是求取量化比例因子，主要通过对称量化、非对称量化方式来求，而找最大值或者阈值的方法又有MinMax、KLD、ADMM、EQ等方法

PTQ Static 算法流程![[Pasted image 20230604235428.png]]  ![[Pasted image 20230604235445.png]]

KL散度校准法：原理
- KL散度校准法也叫相对熵，其中p表示真实分布，q表示非真实分布或p的近似分布：![[Pasted image 20230604235514.png]]
...
![[Pasted image 20230604235827.png]]
...
![[Pasted image 20230604235847.png]]

Quantize量化
- 将float32数据量化为int8。离线转换工具转换的过程之前，根据量化原理的计算出数据量化需要的scale和offset：
![[Pasted image 20230604235915.png]]

Dequantize反量化
- INT8相乘、加之后的结果用INT32格式存储，如果下一Operation需要float32格式数据作为输入，则通过Dequantize反量化操作将INT32数据反量化为float32。Dequantize反量化推导过程如下：
![[Pasted image 20230604235952.png]]

Requantize重量化
- INT8乘加之后的结果用INT32格式存储，如果下一层需要INT8格式数据作为输入，则通过Requantize重量化操作将INT32数据重量化为INT8。重量化推导过程如下：![[Pasted image 20230605000037.png]]
- 其中 y 为下一个节点的输入，即 y=x_next：![[Pasted image 20230605000057.png]]
- 有：![[Pasted image 20230605000110.png]]
...
![[Pasted image 20230605000312.png]]
...
![[Pasted image 20230605000350.png]] ![[Pasted image 20230605000356.png]]
...
#### 模型剪枝分类
![[Pasted image 20230605000718.png]]
...
![[Pasted image 20230605000752.png]]
...
- 进行剪枝 -> 从头训练剪枝后模型
![[Pasted image 20230605000831.png]]
模型剪枝主要单元
...
训练一个模型 -> 对模型进行剪枝 -> 对剪枝后模型进行微调
![[Pasted image 20230605000919.png]]
...
![[Pasted image 20230605001117.png]]   ![[Pasted image 20230605001213.png]]
...
- 知识蒸馏的算法，主要由：1）知识 Knowledge、2）蒸馏算法 Distillate、3）师生架构三个关键部分组成：![[Pasted image 20230605001502.png]]



### Knowledge Format -  蒸馏的知识形式
![[Pasted image 20230605001648.png]]

Response-Based Knowledge
- 主要指Teacher Model 教师模型输出层的特征。主要思想是让 Student Model 学生模型直接学习教师模式的预测结果（Knowledge）。
- 假设张量 z_t 为教师模型输出，张量 z_s 为学生模型输出，Response-based knowledge 蒸馏形式可以被描述为：![[Pasted image 20230605001723.png]]
- Response-based knowledge 主要指 Teacher Model 教师模型最后一层 —— 输出层的特征。其主要思想是让学生模型直接模仿教师模式的最终预测：![[Pasted image 20230605001750.png]]


Feature-Based Knowledge
- 深度神经网络善于学习到不同层级的表征，因此中间层和输出层的都可以被用作知识来训练学生模型，中间层学习知识的 Feature-Based Knowledge 对于 Response-Based Knowledge是一个很好的补充，其主要思想是将教师和学生的特征激活进行关联起来。Feature-Based Knowledge 知识转移的蒸馏损失可表示为：![[Pasted image 20230605001817.png]]
- 虽然基于特征的知识迁移为学生模型的学习提供了良好的信息，但如何有效地从教师模型中选择提示层，从学生模型中选择引导层，仍有待进一步研究。由于提示层和引导层的大小存在显著差异，如何正确匹配教师和学生的特征表示也需要探讨。![[Pasted image 20230605001838.png]]

Relation-Based Knowledge
- 基于 Feature-Based Knowledge 和 Response-Based Knowledge 知识都使用了教师模型中特定层中特征的输出。基于关系的知识进一步探索了不同层或数据样本之间的关系。一般情况下，基于特征图关系的关系知识的蒸馏损失可以表示为：![[Pasted image 20230605001906.png]]
- 传统的知识转移方法往往涉及到个体知识的提炼。教师的个人软标签 Soft Label 被直接提炼到学生中，实际上经过提炼的知识不仅包含了特征信息，还包含了数据样本之间的相互关系。![[Pasted image 20230605001925.png]]
...
![[Pasted image 20230605002010.png]]


Offline Distillation
- 大多数蒸馏采用 Offline Distillation，蒸馏过程被分为两个阶段：1）蒸馏前教师模型预训练；2） 蒸馏算法迁移知识。因此 Offline Distillation 主要侧重于知识迁移部分。
- 通常采用单向知识转移和两阶段训练过程。在步骤1）中需要教师模型参数量比较大，训练时间比较长，这种方式对学生模型的蒸馏比较高效。![[Pasted image 20230605002103.png]]
- Cons：这种训练模式下的学生模型往往过度依赖于教师模型

Online Distillation
Online Distillation 主要针对参数量大、精度性能好的教师模式不可获得的情况。教师模型和学生模型同时更新，整个知识蒸馏算法是一种有效的端到端可训练方案。![[Pasted image 20230605002135.png]]
Cons：现有的 Online Distillation 往往难以获得在线环境下参数量大、精度性能好的教师模型。

Self-Distillation
- 教师模型和学生模型使用相同的网络结构，同样采样端到端可训练方案，属于 Online Distillation 的一种特例。![[Pasted image 20230605002200.png]]
...
![[Pasted image 20230605002308.png]]

Softmax with Temperature

- softmax函数：![[Pasted image 20230605002336.png]]
- 使用软标签就是修改了softmax函数，增加温度系数T：![[Pasted image 20230605002352.png]]
...
![[Pasted image 20230605002442.png]]
...
- 论文中，Hinton 将问题限定在分类问题下，或者属于分类的问题，共同点是模型最后有 softmax 层，其输出值对应类别的概率值。知识蒸馏时，由于已经有一个泛化能力较强的 Net-T，在利用 Net-T 来蒸馏 Net-S 时，可以直接让 Net-S 去学习 Net-T 的泛化能力。![[Pasted image 20230605002623.png]]
1. 训练 Teacher Model；
...
- 训练 Net-T 的过程中，高温蒸馏过程的目标函数由distill loss（对应soft target）和student loss（对应hard target）加权得到：![[Pasted image 20230605002721.png]]
