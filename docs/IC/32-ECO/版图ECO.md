...
前端主要是关注功能，后端主要是关注实现，对于下面的几个主题的关注度，这里展示一下：![12wq](12wq.png)
...
不要慌张，打开脚本一探究竟！一起来看一下ECO的脚本，eco_change.tcl![6453540](6453540.png)
...
![64111110](64111110.png)**
**在ICC里边，就会相应的报告出来tie connection：
![642222220](642222220.png)**
...
为了有效解决这两个问题，这里隆重介绍两位大侠，有了他们护法，上述的问题可以迎刃而解（前提是原始layout 网表里边没有leaf cell的floating input ）![62333440](62333440.png)



这两个变量在icc里边，默认都是false，在进行eco_netlist的时候，需要把他们设置成true（如上图）。

1. 变量一（eco_netlist_ignore_tie_changes）: 设置为true的时候，所有网表里边带来的tie connection，都被eco_netlist忽略，开一下这个选项打开时的ECO 命令的summary:![66666640](66666640.png)

   和原始的ECO脚本相比，减少了很多connect_net的命令，create_net也减少了，最重要的是，create_net -tie 是没有的

   ![64777770](64777770.png)
...
2. 变量二（eco_netlist_preprocess_for_verilog）：这个变量可以忽略网表里边的SYNOPSYS_UNCONNECT，首先这些连接不会对数据库产生任何影响，但是如果需要ICC处理的话，他的方法也很简单，把连接SYNOPSYS_UNCONNECT的pin都会再（append）连接到一个新的net上，名字和这个pin一样，
   *PS：这个做法真的有意义吗？需要问问ICC的开发人员了*![64321232222666770](64321232222666770.png)

   从上图可以看到，eco应用以后，多连了一个net，其实是没有任何用处的。
   所以，在把第二个变量改成true，ECO脚本的真实面貌就呈现出来了，其实这个ECO真的很简单！![643333322222210](643333322222210.png)
...
- add buffer
  ...
  ![在这里插入图片描述](20200411184341144.png)
  ECO的简明流程如下：
  ![在这里插入图片描述](20200411184719182.png)
...
![在这里插入图片描述](20200411191128518.png)
...
- CMD: legalize_placement -increment
  ![在这里插入图片描述](2020041119320558.png)
...
![在这里插入图片描述](20200411194843203.png)
...
![在这里插入图片描述](20200411200717597.png)
...
![在这里插入图片描述](20200411200952723.png)
...
![在这里插入图片描述](20200817190438668.png)
...
![在这里插入图片描述](20200817160900958.png)
...
![在这里插入图片描述](20200817183226859.png)
