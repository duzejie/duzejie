...
-   编写 [Checker]，即 [Special Judge](special-judge.md)。
...
前置知识：[通用](docs/software-engineering/04-cpp/测试-testlib/TestLib.md)
...
Interactor，即交互器，用于交互题与选手程序交互。交互题的介绍见 [题型介绍 - 交互题](算法竞赛-题型概述.md#)。

> [!note]
    Testlib 仅支持 Codeforces 形式交互题，即两程序交互。不支持 NOI 形式的选手编写函数与其他函数交互。

请在阅读下文前先阅读 [通用](docs/software-engineering/04-cpp/测试-testlib/TestLib.md)。
...
请确保 testlib.h 是你 include 的 **第一个** 头文件，Testlib 会重写/禁用（通过名字冲突的方式）一些与随机有关的函数（如 `random()`），保证随机结果与环境无关，这对于 generator 非常重要，[generator 页面](docs/software-engineering/04-cpp/测试-testlib/TestLib.md) 会详细说明这一点。
