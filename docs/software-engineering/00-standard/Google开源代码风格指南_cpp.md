...
mathjax: false
summary: >-
  每个较大的开源项目都有自己的风格指南: 关于如何为该项目编写代码的一系列约定 (有时候会比较武断). 当所有代码均保持一致的风格,
  在理解大型代码库时更为轻松.
categories:
  - 软件工程
  - Cpp
...
| 版本:     | 4.45                                                         |
| --------- | ------------------------------------------------------------ |
| 原作者:   | Benjy Weinberger Craig Silverstein Gregory Eitzmann Mark Mentovai Tashana Landray |
...
所有头文件要能够自给自足。换言之，用户和重构工具不需要为特别场合而包含额外的头文件。详言之，一个头文件要有 [1.2. #define 保护](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#define-guard)，统统包含它所需要的其它头文件，也不要求定义任何特别 symbols.
...
至于什么时候包含头文件，参见 [1.5. #include 的路径及顺序](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#name-and-order-of-includes) 。
...
您所依赖的符号 (symbols) 被哪些头文件所定义，您就应该包含（include）哪些头文件，[前置声明](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#forward-declarations) (forward declarations) 情况除外。比如您要用到 `bar.h` 中的某个符号, 哪怕您所包含的 `foo.h` 已经包含了 `bar.h`, 也照样得包含 `bar.h`, 除非 `foo.h` 有明确说明它会自动向您提供 `bar.h` 中的 symbol. 不过，凡是 cc 文件所对应的「相关头文件」已经包含的，就不用再重复包含进其 cc 文件里面了，就像 `foo.cc` 只包含 `foo.h` 就够了，不用再管后者所包含的其它内容。
...
> 根据下文将要提到的策略合理使用命名空间.
>
> - 遵守 [命名空间命名](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/naming.html#namespace-names) 中的规则。
>
...
> 有时, 把函数的定义同类的实例脱钩是有益的, 甚至是必要的. 这样的函数可以被定义成静态成员, 或是非成员函数. 非成员函数不应依赖于外部变量, 应尽量置于某个命名空间内. 相比单纯为了封装若干不共享任何静态数据的静态成员函数而创建类, 不如使用 [2.1. 命名空间](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#namespaces) 。举例而言，对于头文件 `myproject/foo_bar.h` , 应当使用
>
> ```cpp
> namespace myproject {
> namespace foo_bar {
...
>
> 如果你必须定义非成员函数, 又只是在 `.cc` 文件中使用它, 可使用匿名 [2.1. 命名空间](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#namespaces) 或 `static` 链接关键字 (如 `static int Foo() {...}`) 限定其作用域.
...
- 在没有使程序崩溃 (因为并不是一个始终合适的方法) 或者使用异常 (因为已经被 [禁用](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#exceptions) 了) 等方法的条件下, 构造函数很难上报错误
...
注意: 类和结构体的成员变量使用不同的 [命名规则](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#variable-names).
...
当子类继承基类时, 子类包含了父基类所有数据及操作的定义. C++ 实践中, 继承主要用于两种场合: 实现继承, 子类继承父类的实现代码; [接口继承](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#interface), 子类仅继承父类的方法名称.
...
对于可能被子类访问的成员函数, 不要过度使用 `protected` 关键字. 注意, 数据成员都必须是 [私有的](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#access-control).
...
真正需要用到多重实现继承的情况少之又少. 只在以下情况我们才允许多重继承: 最多只有一个基类是非抽象类; 其它基类都是以 `Interface` 为后缀的 [纯接口类](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#interface).
...
相比单继承 (见 [继承](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#inheritance)), 多重实现继承可以复用更多的代码.
...
只有当所有父类除第一个外都是 [纯接口类](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#interface) 时, 才允许使用多重继承. 为确保它们是纯接口, 这些类必须以 `Interface` 为后缀.

**注意**

关于该规则, Windows 下有个 [特例](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#windows-code).
...
以 `Interface` 为后缀可以提醒其他人不要为该接口类增加函数实现或非静态数据成员. 这一点对于 [多重继承](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#multiple-inheritance) 尤其重要. 另外, 对于 Java 程序员来说, 接口的概念已是深入人心.
...
类型转换运算符在 [隐式类型转换](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#implicit-conversions) 一节有提及. `=` 运算符在 [可拷贝类型和可移动类型](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#copyable-and-movable-types) 一节有提及. 运算符 `<<` 在 [流](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#streams) 一节有提及. 同时请参见 [函数重载](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#function-overloading) 一节, 其中提到的的规则对运算符重载同样适用.
...
将 *所有* 数据成员声明为 `private`, 除非是 `static const` 类型成员 (遵循 [常量命名规则](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#constant-names)). 处于技术上的原因, 在使用 [Google Test](https://github.com/google/googletest) 时我们允许测试固件类中的数据成员为 `protected`.
...
不要将大段的函数定义内联在类定义中. 通常，只有那些普通的, 或性能关键且短小的函数可以内联在类定义中. 参见 [内联函数](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#inline-functions) 一节.
...
如果打算重载一个函数, 可以试试改在函数名里加上参数信息. 例如, 用 `AppendString()` 和 `AppendInt()` 等, 而不是一口气重载多个 `Append()`. 如果重载函数的目的是为了支持不同数量的同一类型参数, 则优先考虑使用 `std::vector` 以便使用者可以用 [列表初始化](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#braced-initializer-list) 指定参数.

#### 4.5. 缺省参数

**总述**

只允许在非虚函数中使用缺省参数, 且必须保证缺省参数的值始终一致. 缺省参数与 [函数重载](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#function-overloading) 遵循同样的规则. 一般情况下建议使用函数重载, 尤其是在缺省函数带来的可读性提升不能弥补下文中所提到的缺点的情况下.
...
缺省参数实际上是函数重载语义的另一种实现方式, 因此所有 [不应当使用函数重载的理由](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#function-overloading) 也都适用于缺省参数.
...
后置返回类型是显式地指定 [Lambda 表达式](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#lambda-expressions) 的返回值的唯一方式. 某些情况下, 编译器可以自动推导出 Lambda 表达式的返回类型, 但并不是在所有的情况下都能实现. 即使编译器能够自动推导, 显式地指定返回类型也能让读者更明了.
...
在大部分情况下, 应当继续使用以往的函数声明写法, 即将返回类型置于函数名前. 只有在必需的时候 (如 Lambda 表达式)  或者使用后置语法能够简化书写并且提高易读性的时候才使用新的返回类型后置语法. 但是后一种情况一般来说是很少见的,  大部分时候都出现在相当复杂的模板代码中, 而多数情况下不鼓励写这样 [复杂的模板代码](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#template-metaprogramming).
...
>
> 对于 Windows 代码来说, 有个 [特例](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#windows-code).
...
>
> 至于 `dynamic_cast` 参见 [6.8. 运行时类型识别](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#rtti).
...
至于格式化，参见 [9.7. 列表初始化格式](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#braced-initializer-list-format).
...
>
> 和 [6.23. Boost 库](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#boost) 一样，有些 C++11 扩展提倡实则对可读性有害的编程实践——就像去除冗余检查（比如类型名）以帮助读者，或是鼓励模板元编程等等。有些扩展在功能上与原有机制冲突，容易招致困惑以及迁移代价。
...
6. 由于友元函数／类并不是类的一部分，自然也不会是类可调用的公有接口，于是我主张全集中放在类的尾部，即的数据成员之后，参考 [声明顺序](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#declaration-order) 。
...
模板参数的命名应当遵循对应的分类: 类型模板参数应当遵循 [类型命名](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#type-names) 的规则, 而非类型模板应当遵循 [变量命名](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#variable-names) 的规则.
...
C++ 文件要以 `.cc` 结尾, 头文件以 `.h` 结尾. 专门插入文本的文件则以 `.inc` 结尾, 参见 [头文件自足](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#self-contained-headers).
...
结构体与类的使用讨论, 参考 [结构体 vs. 类](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#structs-vs-classes).
...
注意 [不使用缩写作为名称](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#general-naming-rules) 的规则同样适用于命名空间. 命名空间中的代码极少需要涉及命名空间的名称, 因此没有必要在命名空间中使用缩写.
...
枚举的命名应当和 [常量](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#constant-names) 或 [宏](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#macro-names) 一致: `kEnumName` 或是 `ENUM_NAME`.

**说明**

单独的枚举值应该优先采用 [常量](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#constant-names) 的命名方式. 但 [宏](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#macro-names) 方式的命名也可以接受. 枚举名 `UrlTableErrors` (以及 `AlternateUrlTableErrors`) 是类型, 所以要用大小写混合的方式.
...
2009 年 1 月之前, 我们一直建议采用 [宏](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#macro-names) 的方式命名枚举值. 由于枚举值和宏之间的命名冲突, 直接导致了很多问题. 由此, 这里改为优先选择常量风格的命名方式. 新代码应该尽可能优先使用常量风格. 但是老代码没必要切换到常量风格, 除非宏风格确实会产生编译期问题.
...
你并不打算 [使用宏](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#preprocessor-macros), 对吧? 如果你一定要用, 像这样命名: `MY_MACRO_THAT_SCARES_SMALL_CHILDREN`.

**说明**

参考 [预处理宏](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#preprocessor-macros); 通常 *不应该* 使用宏. 如果不得不用, 其命名像枚举命名一样全部大写, 使用下划线:
...
[头文件保护](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#define-guard) 可以无视该原则.
...
返回类型和函数名在同一行, 参数也尽量放在同一行, 如果放不下就对形参分行, 分行方式与 [函数调用](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#function-calls) 一致.
...
您平时怎么格式化函数调用, 就怎么格式化 [列表初始化](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#braced-initializer-list).
...
如果一个布尔表达式超过 [标准行宽](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#line-length), 断行方式要统一一下.
...
类声明 (下面的代码中缺少注释, 参考 [类注释](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#class-comments)) 的基本格式如下:
...
- 关于声明顺序的规则请参考 [声明顺序](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#declaration-order) 一节.
...
[命名空间](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#namespaces) 不要增加额外的缩进层次, 例如:
...
- 不要使用 `#pragma once`; 而应该使用 Google 的头文件保护规则. 头文件保护的路径应该相对于项目根目录 (Yang.Y 注: 如 `#ifndef SRC_DIR_BAR_H_`, 参考 [#define 保护](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#define-guard) 一节).
...
- 通常我们 [禁止使用多重继承](file:///home/duzejie/%E4%B8%8B%E8%BD%BD/zh-google-styleguide-latest/contents.html#multiple-inheritance), 但在使用 COM 和 ATL/WTL 类时可以使用多重继承. 为了实现 COM 或 ATL/WTL 类/接口, 你可能不得不使用多重实现继承.
