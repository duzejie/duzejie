...
## Transformer 整体结构
* 添加了补充信息（手动修改版）：
![Img](attachments/01.Transformer/77eddfd2576bc6b10f95e99d565f7e2c_MD5.png)
...
* 公式：
  ...
  \end{aligned}
  $$
  ![Img](attachments/01.Transformer/9177061605cc8a42fe32d972176c6e10_MD5.png)
...
* 公式： x = 单词 Embedding + 位置 Embedding
  ![Img](attachments/01.Transformer/ddd5a6bef3c6f98725d989195860d089_MD5.png)
...
* 假设我们有一个模型，将英语句子$X$翻译成法语句子$Y$
    * $X$的第一个词向量一般是$BOS$/$SOS$标记，该模型需要从输入句子 $X$ 中为输出句子 $Y$ 中的第一个词向量 $y1$ 提取上下文 
      ...
      $$
    * 对于$Q$:
        * 假设：$y_1$是输出句子$Y$的第一个元素，本身是四个维度的向量![Img](attachments/01.Transformer/8baece6b0e4611a7d8865c960ab3bb00_MD5.png)
        * 假设：$Q$的权重矩阵为$W^Q$，![Img](attachments/01.Transformer/abe36b8751c62979adf3cb2d7bd63c5f_MD5.png)
        * 对$y_1$进行$W^Q$点积计算$q_1 = y_1 \cdot W^Q$，计算$y_1$在注意力计算中的查询需求：![Img](attachments/01.Transformer/b5c6941024d4414e43872685968b341b_MD5.png)
        * 对于其他q值
          $$ Q = \begin{bmatrix}
                \boldsymbol{q}_1 \\
                \boldsymbol{q}_2 \\
    ...
    * 对于$K$:
        ...
        * 假设：$K$的权重矩阵为$W^K$
        * 对$x_1$进行$W^K$点积计算$k_1 = x_1 \cdot W^K$，计算$x_1$在注意力计算中的查询需求![Img](attachments/01.Transformer/d2d3568585bf2a300cc0ced7eeb13fc4_MD5.png)
        * 对于其他k值
          $$
          K = \begin{bmatrix}
            \boldsymbol{k}_1 \\
...
* 结构示意图：
  ...
  ![Img](attachments/01.Transformer/31c8c5899e1fa922dac92cc06a678f82_MD5.png)

* 计算过程图：

  ![Img](attachments/01.Transformer/a25136da38420ef91517c7aab0c5026b_MD5.png)
  ![Img](attachments/01.Transformer/47b0e4bc5c4e17400183e06b18f8b6bf_MD5.png)
  ![Img](attachments/01.Transformer/793e0e021fa32a0c42b6d9be398fcc4c_MD5.png)
...
* 由于每个头的维度降低，总计算成本与具有全维度的单头注意力相似
  ...
  $W^{O}$：通过变换还原对应的维度的矩阵
* 结构示意图：
  ![Img](attachments/01.Transformer/993224101e81aac3ad802d700d25223f_MD5.png)
* 计算示意图：
  ![Img](attachments/01.Transformer/f5e4bf1b18054f03b716505d50df89a8_MD5.png)
  ![Img](attachments/01.Transformer/39860101e8a62e516cd1e42c5bd1d94d_MD5.png)
...
* 残差连接公式：![Img](attachments/01.Transformer/97c0c4c553efe5123edd6192651f20c1_MD5.png)
...
* Mask预测过程：
    * 对$QK^{\mathrm{T}}$的结果应用Mask矩阵转换
      ![Img](attachments/01.Transformer/9dcaa0765e6c95fedf2579c48d24e07c_MD5.png)
